...
- [ ] **People rendered via `Avatar`** — every person (patient, operator, staff, signed-in user, list row, comment author) uses the kit `Avatar` for the visual identity, never a hand-rolled coloured circle, hand-written initials, or a raw `<img>`. And the name is shown in **one consistent format** throughout — no mixing "John Doe" / "J. Doe" / "Doe, John" across (or within) a screen. Hand-rolled person avatar → **blocker**; inconsistent name formats → **warning**.
